...
public class App{
	...
	public static void main(String[] args) throws Exception{
		...
		Credentials credentials = accountService.findFromConfigOption("deploy").orElseGet(() -> {
		...
		if(args.length > 0){
			...
			logger.info("Load Hotel2000 contract: " + hotel2000ContractAddress);
			hotel2000 = Hotel2000.load(hotel2000ContractAddress, web3j, credentials, new DefaultGasProvider());
		} else {
			...
			hotel2000 = Hotel2000.deploy(web3j, credentials, new DefaultGasProvider()).send();
		...
		env = ConsoleEnv.builder()
				.web3j(web3j)
				.currantContract(hotel2000ContractAddress)
				.build();
